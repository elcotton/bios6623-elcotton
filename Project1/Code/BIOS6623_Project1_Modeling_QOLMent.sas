...
*QOL-Mental as outcome FULL MODEL;
proc mcmc data = hiv nbi = 30000 nmc = 100000 plots = all DIC;
	...
	mu = betaint + betaSmokerInd*SmokerInd + betaDrinksInd*DrinksInd + betaEducationInd*EducationInd + betaAdherenceInd*AdherenceInd
			...
			betaAge*Age + betaHardDrugs*HardDrugs + betaQOLBase*QOL_MentBase;
	model QOL_MentDiff ~ normal(mu, var = sigma2);
	title "Model 1: Full model with QOL_MentDiff and All variables";
run;

*QOL-Mental as outcome CRUDE MODEL;
proc mcmc data = hiv nbi = 30000 nmc = 100000 plots = all DIC;
	parms betaint 0  betaHardDrugs 0 betaQOLBase 0;
	parms sigma2 1;
	prior betaint betaHardDrugs betaQOLBase ~ normal(mean = 0, var = 1000);
	prior sigma2 ~igamma(shape = 2.001, scale = 1.001);
	mu = betaint + betaHardDrugs*HardDrugs + betaQOLBase*QOL_MentBase;
	model QOL_MentDiff ~ normal(mu, var = sigma2);
	title "Model 1: Crude model with QOL_MentDiff and All variables";
run;

*Crude and Full don't change enough! DONE! GOLDEN!;
